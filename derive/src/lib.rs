...
pub fn encode_derive(input: TokenStream) -> TokenStream {
	...
	let generics = add_trait_bounds(input.generics, parse_quote!(_susy_codec::Encode));
	...
	let generated = quote! {
		...
		const #dummy_const: () = {
			...
			extern crate susy_codec as _susy_codec;
			#impl_block
		};
	};
...
pub fn decode_derive(input: TokenStream) -> TokenStream {
	...
	let generics = add_trait_bounds(input.generics, parse_quote!(_susy_codec::Decode));
	let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
